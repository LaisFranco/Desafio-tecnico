...
RBAC (Role-Based Access Control), ou Controle de Acesso Baseado em Papéis, é um modelo no qual permissões são atribuídas a papéis, e os usuários herdam essas permissões ao serem associados a esses papéis.

Nesse modelo, os papéis representam responsabilidades dentro do sistema, e não ações isoladas.  
Essa abordagem garante maior segurança, escalabilidade e aderência ao princípio do menor privilégio.

No contexto deste case, o problema identificado estava na modelagem dos papéis atribuídos ao usuário Taylor, e não em uma permissão ausente. Por isso, a solução foi aplicada no nível de roles, e não diretamente no usuário.
